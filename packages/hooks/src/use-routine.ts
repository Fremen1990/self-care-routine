...
export function useRoutine() {
  ...
  const resetMorning = useCallback(() => {
    setMorningTasks(prev => prev.map(task => ({ ...task, completed: false })));
  }, []);

  const resetEvening = useCallback(() => {
    setEveningTasks(prev => prev.map(task => ({ ...task, completed: false })));
  }, []);
  ...
  const morningProgress = useMemo(() =>
    RoutineService.calculateProgress(morningTasks), [morningTasks]
  );

  const eveningProgress = useMemo(() =>
    RoutineService.calculateProgress(eveningTasks), [eveningTasks]
  );

  const sleepTimes = useMemo(() => {
    const firstTask = morningTasks[0];
    return firstTask ? RoutineService.calculateSleepTimes(firstTask.time) : { sevenHalf: '', eight: '' };
  }, [morningTasks]);
